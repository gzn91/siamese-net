...
def fc(x, units, name, activation_fn=tf.nn.relu, l2reg=0.0):
    with tf.variable_scope(name):
        ...
        tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES, l2reg*tf.nn.l2_loss(w))
        h = tf.matmul(x, w) + b
...
def conv2d(x, filters, name, kernel_size=3, strides=1, padding='VALID',
           activation_fn=tf.nn.relu, training=True, use_bn=False, l2reg=0.0):
    ...
    with tf.variable_scope(name):
        ...
        tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES, l2reg*tf.nn.l2_loss(wx))
        if use_bn:
            x = tf.layers.batch_normalization(x, training=training)
    return activation_fn(x)


def conv2d_transpose(x, filters, name, kernel_size=3, strides=2, padding='VALID',
                     activation_fn=tf.nn.relu, training=True, use_bn=False, l2reg=0.0):
    nb, nw, nh, nc = x.get_shape().as_list()
    if padding == 'VALID':
